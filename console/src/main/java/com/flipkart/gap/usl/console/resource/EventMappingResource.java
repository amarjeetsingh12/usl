    ...
    public Response createEventMapping(@Valid EventMapping eventMapping) {
        try {
            if(eventMapping.getMapping().findValues("entityId") == null) {
                log.error("Error creating Event Mapping EntityId Does not exist in mapping", eventMapping.getMapping());
            ...
            log.error("Error creating Event Mapping Already exist {} ", eventMapping.getSourceEventId(), eventMapping.getEventType(), e);
